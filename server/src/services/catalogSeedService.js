...
export const DEMO_SELLER_EMAIL = 'seed.seller@bmobile.demo';
...
const ensureDemoSeller = async (logger) => {
  ...
  if (!seller) {
    ...
    logInfo(logger, `Created demo seller: ${DEMO_SELLER_EMAIL}`);
    return seller;
  }

  if (seller.sellerProfile?.status !== SELLER_STATUS.APPROVED) {
    ...
    await seller.save();
    logInfo(logger, `Updated seller to APPROVED: ${DEMO_SELLER_EMAIL}`);
  }
...
export const seedDemoCatalog = async ({
  ...
  try {
    if (onlyIfEmpty) {
      const existingPublicCount = await Product.countDocuments({
        isActive: true,
      ...
      if (existingPublicCount > 0) {
        logInfo(
          logger,
          `Skipped demo catalog seeding because ${existingPublicCount} published products already exist.`
        );
        return {
          skipped: true,
    ...
    const seller = await ensureDemoSeller(logger);
    ...
    if (overwrite) {
      ...
      if (deleted.deletedCount) {
        logInfo(logger, `Removed ${deleted.deletedCount} previous B-Mobile seed products.`);
      }
    }
    ...
    logInfo(
      logger,
      `Seeded ${demoCatalogItems.length} products across phones, tablets, and laptops.`
    );
    logInfo(logger, `Seller login: ${DEMO_SELLER_EMAIL} / DemoSeller@123`);

    return {
      skipped: false,
      seededCount: demoCatalogItems.length,
      sellerEmail: DEMO_SELLER_EMAIL,
    };
  } finally {
